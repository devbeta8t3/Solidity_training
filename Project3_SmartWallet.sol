...
contract SmartWallet {
    ...
    mapping(address => bool) public isAllowdToSend;
    ...
    function proposeNewOwner(address payable newOwner) public {
        require(guardian[msg.sender], "You are so guardian, aborting!");
    ...
    function setAllowance(address _from, uint _amount) public {
        ...
        isAllowdToSend[_from] = true;
    }

    function denySending(address _from) public {
        require(msg.sender == owner, "You are not the owner, aborting!");
        isAllowdToSend[_from] = false;
    }
    ...
    function transfer(address payable _to, uint _amount, bytes memory payload) public returns (bytes memory) {
        ...
        if (msg.sender != owner) {
            require(isAllowdToSend[msg.sender], "You are not allowed to send any transactions, aborting.");
